...
Raket.Controls = (function () {
    ...
    ControlClass.prototype.buttonPressed = function (b) {
        if (typeof(b) == 'object') {
            return b.pressed
        }
        return b == 1.0
    }
    ControlClass.prototype.controllerLoop = function () {
        var gamepads = navigator.getGamepads ? navigator.getGamepads() : (navigator.webkitGetGamepads ? navigator.webkitGetGamepads : [])
        if (!gamepads) {
            return
        }
        ...
        this.keyMap[32] = this.buttonPressed(gp.buttons[0])

        this.keyMap[37] = axes[0] < -0.1 || this.buttonPressed(gp.buttons[14]); // LEFT
        this.keyMap[39] = axes[0] > 0.1 || this.buttonPressed(gp.buttons[15]); // LEFT
        this.keyMap[40] = axes[1] > 0.1 || this.buttonPressed(gp.buttons[13]) // DOWN
        this.keyMap[38] = axes[1] < -0.1 || this.buttonPressed(gp.buttons[12]) // UP
